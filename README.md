...
- **FDE 入场包**：60 分钟，读你的聊天记录导出（没有就访谈你），把你的高频工作流写成 AI 可接管的交接包。
...
FDE 入场包是一套 AI 交接资产：让 Fable 5 做你的第一任 FDE 工程师——它退场后，你自己的（更弱的）模型拿着包接管日常。
...
把这段粘给能执行代码的 agent（Claude Code / Codex / Cowork）：

```
帮我跑 github.com/ShaohuaDavidLee/simaqian 的 FDE 入场包（输出三）。
如果还没装，先 clone 到 ~/.claude/skills/simaqian.skill。
我的材料在 ./me/，聊天记录导出在 ./export/（没有就轻量模式，访谈我）。
