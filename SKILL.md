...
开始前先告诉用户：可以直接给你文件、文字或链接；不要求用户把材料放进固定目录，除非用户正在 Claude Code / Codex 等本地开发环境里，并且已经说明了文件路径。

优先读取用户已经给出的材料。精列传最适合读取这四类：
...
如果用户没有提供任何材料，先简短提醒他可以提供上面四类材料；如果用户仍然没有材料，或明确说“访谈我”，再进入访谈模式。

访谈时每轮最多问 1-3 个问题，不要一次发长问卷。需要完整采集时，优先从下面这些高杠杆问题中挑选，而不是机械全问：
