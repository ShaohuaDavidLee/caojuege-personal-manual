...
核心逻辑：顶级模型是请来的资深 FDE 工程师，随时可能撤场；退场前让它留下一套**你自己的（可能更弱的）模型能接管**的交接包。所有资产写到弱模型可执行的程度；交接考试用接管的那个便宜模型来考，不用建包的资深考。

前置：已有 `persona-agent.md`（没有则先跑精列传或做压缩访谈）。有聊天记录导出走完整模式（约 60 分钟，需可执行代码的 agent）；没有走轻量模式（约 30 分钟，访谈）。
