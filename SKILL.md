...
前置：已有 `persona-agent.md`（没有则先跑精列传或做压缩访谈）。默认走轻量模式，只使用本轮明确提供的材料、已有 `persona-agent.md` 和用户明确指定的项目文件；材料不足时最多追问 5 个问题。只有当用户明确选择证据版，并主动提供材料目录时，才进入 [references/fde-pack.md](references/fde-pack.md) 里的证据版工作流。
