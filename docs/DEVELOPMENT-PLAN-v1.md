...
- 导航：司马迁.skill · 开源 · 草诀歌之笔
- 标题：司马迁.skill
- 一句话：把你是谁，整理成一式两份。
- 副句：一份给 agent 的 persona，一份给人的个人主页。草诀歌之笔 · 第一件。
- 产物示意：persona.md 与主页并排（可沿用现有 `.mock-md` / `.mock-browser` 纯 CSS 模拟，或用 `examples/screenshots/` 静态图；二选一，不要两者都放）
- 一个提示词面板 + 一个复制按钮（精列传启动提示词）
- 一行小字：已有 persona？更新方法见 README
- 页脚：开源 GitHub · 草诀歌之笔 · 宣言（指向 GitHub 上的 MANIFESTO.md，不再是 `#`）
...
- 首页 Lighthouse 无外部请求，移动端首屏能看到产物示意和复制按钮。
